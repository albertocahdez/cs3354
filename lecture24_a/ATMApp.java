	...
	private void setCenterPanel() {
		...
		for (int i = 1; i < 10; i++) {
			...
			button.addActionListener(e -> {
				if (inputBox.getText().length() <= 6)
					inputBox.setText(inputBox.getText() + button.getText());
			});
			keyPad.add(button);
		}
		...
		buttonA.addActionListener(e -> {
			if (theATM.getState() == ATM.START) {
				if (inputBox.getText().length() == 0) {
					JOptionPane.showMessageDialog(this, "No customer number provided.", "Error",
				...
				if (inputBox.getText().length() == 0) {
					JOptionPane.showMessageDialog(this, "No customer PIN provided.", "Error",
							JOptionPane.ERROR_MESSAGE);
				} else {
				...
				setGUIState(theATM.getState());
			} else if (theATM.getState() == ATM.TRANSACT_DEPOSIT
					|| theATM.getState() == ATM.TRANSACT_WITHDRAW) {
				if (inputBox.getText().length() == 0) {
					JOptionPane.showMessageDialog(this, "No transaction amount provided", "Error",
							JOptionPane.ERROR_MESSAGE);
				} else {
